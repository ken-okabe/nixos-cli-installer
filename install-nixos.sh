...
# 1.2. Partitioning Constants (Swap size is fixed based on prior discussion)
SWAP_SIZE_GB="16"
...
# 1.3. System Username
echo ""
read -r -p "Enter the desired username for the primary system user (e.g., ken, nixosuser): " NIXOS_USERNAME
...
# Generate password hash using mkpasswd (should be available in NixOS live env).
...
if [[ -z "$PASSWORD_HASH" || ! "$PASSWORD_HASH" == \$6\$* ]]; then # Basic check for $6$ hash format.
    echo "Error: Failed to generate password hash with mkpasswd."
    echo "       Ensure mkpasswd is available and working correctly."
    exit 1
fi
...
echo "  EFI Size:           $DEFAULT_EFI_SIZE"
...
echo "Step 2: Starting disk operations on $TARGET_DISK..."
echo "This is the most critical part. Please ensure you have selected the correct disk."
if ! confirm "FINAL WARNING: All data on $TARGET_DISK will be erased. Proceed with partitioning?"; then
    echo "Partitioning aborted by user."
    exit 1
fi

{ # Start of a block for disk operations, allows trapping errors for this section.
    ...
    sudo sgdisk -p "$TARGET_DISK" # Display initial empty state.

    echo "Creating EFI System Partition (ESP) at the start of the disk..."
    sudo sgdisk --new=1:0:+"$DEFAULT_EFI_SIZE" --typecode=1:ef00 --change-name=1:"$EFI_PART_NAME" "$TARGET_DISK"
    echo "EFI partition created."
    sudo sgdisk -p "$TARGET_DISK" # Display state after EFI creation.

    echo "Creating SWAP partition at the end of the disk (${SWAP_SIZE_GB}GiB)..."
    # Partition number 3 is used here to logically place it for sgdisk,
    # then partition 2 will fill the gap. Actual /dev/sdXN numbers are by physical order.
    sudo sgdisk --new=3:0:-"${SWAP_SIZE_GB}G" --typecode=3:8200 --change-name=3:"$SWAP_PART_NAME" "$TARGET_DISK"
    echo "SWAP partition defined at the end."
    sudo sgdisk -p "$TARGET_DISK" # Display state after SWAP creation.

    echo "Creating ROOT partition in the largest remaining space (between EFI and SWAP)..."
    ...
    sudo sgdisk -p "$TARGET_DISK" # Display final layout.

    echo "Informing kernel of partition table changes..."
    sudo partprobe "$TARGET_DISK" && sleep 3 # Give kernel time to recognize new partitions.

    # Verify that partlabels are available before formatting
    ...
    if [ ! -e "/dev/disk/by-partlabel/$EFI_PART_NAME" ] || \
       ...
       [ ! -e "/dev/disk/by-partlabel/$SWAP_PART_NAME" ]; then
        echo "ERROR: Not all partition labels are accessible under /dev/disk/by-partlabel/."
        echo "       Please check the partition creation steps and labels."
    ...
    sudo mount -L "$ROOT_PART_NAME" /mnt # Mount root by label
    sudo mkdir -p /mnt/boot
    sudo mount -L "$EFI_PART_NAME" /mnt/boot # Mount EFI by label
    sudo swapon -L "$SWAP_PART_NAME" # Activate swap by label
    ...
    echo "       Please check the messages above. Installation cannot proceed."
    exit 1
}
...
# This command creates /mnt/etc/nixos/hardware-configuration.nix
# and also a default /mnt/etc/nixos/configuration.nix.
...
# The generated configuration.nix is not used directly by our Flake if not listed,
# but it's good to inform the user it exists.
if [ -f "${TARGET_NIXOS_CONFIG_DIR}/configuration.nix" ]; then
    ...
    echo "      Our Flake setup uses custom modules which will supersede most of its content."
fi
echo "--------------------------------------------------------------------"

# --- 4. Generate Flake and Custom Module Files from Templates ---
echo "Step 4: Generating Flake and custom NixOS module files..."
sudo mkdir -p "${TARGET_NIXOS_CONFIG_DIR}" # Ensure target directory exists.
...
generate_from_template() {
    ...
    # Build the sed script with all replacements.
    # Using '|' as the sed delimiter to avoid issues if paths or hashes contain '/'.
    # Placeholders should be unique (e.g., __KEY__).
    local sed_script=""
    ...
    # For bootloader.nix.template, if GRUB is used on a specific disk.
    sed_script+="-e \"s|__TARGET_DISK_FOR_GRUB__|${TARGET_DISK}|g\" "

    # Use a temporary file for sed output, then sudo mv to handle permissions.
    local temp_output
    temp_output=$(mktemp)
...
if [[ -d "$USER_CONFIG_FILES_DIR" ]]; then
    ...
    echo "WARNING: User config file directory ($USER_CONFIG_FILES_DIR) not found. Zellij configs might be missing."
fi
...
if confirm "Proceed with NixOS installation?"; then
    ...
    echo "You can then run 'sudo nixos-install --no-root-passwd --flake ${TARGET_NIXOS_CONFIG_DIR}#${HOSTNAME}' manually if desired."
fi
